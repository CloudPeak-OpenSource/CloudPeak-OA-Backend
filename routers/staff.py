...
async def bind_me(
    ...
    result = await engine.count(Staff, Staff.user_id == token_info["id"])

    if result > 0:
        raise HTTPException(status_code=409, detail="该用户已经绑定了员工")

    try:
        section = await engine.find_one(Section, Section.id == ObjectId(section_id))
    except InvalidId:
    ...
    await engine.save(
        Staff(
            id=ObjectId(), name=name, tags=[], user_id=token_info["id"], section=section
        )
    )

    return engine.find_one(Staff, Staff.user_id == token_info["id"])
